...
RSpec.describe "API::V1::Items", type: :request do
  ...
  describe "POST /api/v1/stores/:store_id/items" do
    ...
    it "returns error for invalid params" do
      post "/api/v1/stores/#{store.id}/items", params: invalid_params
  ...
  describe "PATCH /items/:id" do
    ...
    it "updates an item successfully" do
      patch "/api/v1/items/#{item.id}", params: { item: { name: "Updated item name" } }
      expect(response).to have_http_status(:ok)
      expect(item.reload.name).to eq("Updated item name")
    end

    it "returns 404 not_found for non existent item" do
      patch "/api/v1/items/700005", params: { item: { name: "Does not exist" } }
      expect(response).to have_http_status(:not_found)
    end
  end
